...
export function Latest() {
  ...
  const fetchInitialPrompts = async () => {
    setLoading(true);
    try {
      const q = query(collection(db, 'prompts'), orderBy('createdAt', 'desc'), limit(16));
  ...
  const fetchMorePrompts = async () => {
    ...
    try {
      const q = query(collection(db, 'prompts'), orderBy('createdAt', 'desc'), startAfter(lastVisible), limit(12));
      const snapshot = await getDocs(q);
      const data = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as Prompt));
  ...
  return (
    <div className="min-h-screen bg-brand-bg selection:bg-brand-purple/30">
      ...
      <main className="pt-32 pb-16">
        ...
        >
          <div className="inline-flex items-center justify-center w-16 h-16 rounded-full bg-brand-blue/10 mb-6 border border-brand-blue/20">
          ...
          <p className="text-zinc-400 max-w-2xl mx-auto text-lg">
            Stay ahead of the curve with the freshest prompts added to our ever-growing collection.
          </p>
        </motion.div>
        ...
        <section className="px-4 max-w-7xl mx-auto min-h-[50vh]">
          {prompts.length === 0 && !loading ? (
             <div className="text-center py-32">
                ...
                <h3 className="text-xl font-medium text-white mb-2">No latest prompts</h3>
          ...
          <div ref={ref} className="py-20 flex justify-center">
            {loading && (
              <div className="flex items-center gap-3 bg-brand-secondary/50 px-6 py-3 rounded-full border border-white/5 backdrop-blur-sm">
                ...
                <span className="text-sm font-medium text-zinc-300">Loading fresh ideas...</span>
              </div>
            )}
          </div>
        </section>
